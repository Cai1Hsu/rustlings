...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        let entry1 = scores.entry(team_1_name);

        let value1 = entry1.or_insert(DEFAULT_TEAM);

        value1.goals_scored += team_1_score;
        value1.goals_conceded += team_2_score;

        let entry2 = scores.entry(team_2_name);
 
        let value2 = entry2.or_insert(DEFAULT_TEAM);
